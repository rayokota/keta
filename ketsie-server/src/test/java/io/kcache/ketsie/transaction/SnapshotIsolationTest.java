...
public class SnapshotIsolationTest {
    ...
    private static final Logger LOG = LoggerFactory.getLogger(io.kcache.ketsie.transaction.SnapshotIsolationTest.class);

    @Test
    public void testSnapshotIsolation() throws Exception {
        String userTableName = "MY_TX_TABLE";
        byte[] initialData = new byte[]{0};
        byte[] dataValue1 = new byte[]{1};
        byte[] dataValue2 = new byte[]{2};

        try (TransactionManager tm = KetsieTransactionManager.newInstance()) {
            TxVersionedCache versionedCache = new TxVersionedCache(new VersionedCache(userTableName));
            byte[] rowId = new byte[]{100};
