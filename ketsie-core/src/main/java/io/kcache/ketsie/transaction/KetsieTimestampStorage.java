...
public class KetsieTimestampStorage implements TimestampStorage {

    private static final long INITIAL_MAX_TS_VALUE = 0L;

    private static final Logger LOG = LoggerFactory.getLogger(KetsieTimestampStorage.class);

    private static final long TSO_KEY = -1L;

    private final Cache<Long, Long> cache;

    public KetsieTimestampStorage(Cache<Long, Long> cache) {
        this.cache = cache;
    }
