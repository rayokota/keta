...
public class KetaSyncPostCommitter implements PostCommitActions {

    private static final Logger LOG = LoggerFactory.getLogger(KetaSyncPostCommitter.class);

    private final CommitTable.Client commitTableClient;

    public KetaSyncPostCommitter(CommitTable.Client commitTableClient) {
    ...
    public ListenableFuture<Void> updateShadowCells(AbstractTransaction<? extends CellId> transaction) {
        ...
        KetaTransaction tx = KetaTransactionManager.enforceKetaTransactionAsParam(transaction);

        try {
            Map<String, VersionedCache> caches = new HashMap<>();
            // Add shadow cells
            for (KetaCellId cell : tx.getWriteSet()) {
            ...
            for (KetaCellId cell : tx.getConflictFreeWriteSet()) {
                VersionedCache cache = cell.getCache();
                caches.put(cache.getName(), cache);
                cache.setCommit(tx.getGenerationId(), cell.getKey(), cell.getTimestamp(), tx.getCommitTimestamp());
            }
