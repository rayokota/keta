...
public class KetaTimestampStorage implements TimestampStorage {

    private static final long INITIAL_MAX_TS_VALUE = 0L;

    private static final Logger LOG = LoggerFactory.getLogger(KetaTimestampStorage.class);

    private static final long TSO_KEY = -1L;

    private final Cache<Long, Long> cache;

    public KetaTimestampStorage(Cache<Long, Long> cache) {
        this.cache = cache;
    }
