...
import io.kcache.keta.transaction.client.KetaTransactionManager.CommitTimestampLocatorImpl;
...
public class SnapshotFilterImpl implements SnapshotFilter {
    ...
    private void healShadowCell(KetaTransaction tx, KeyValue<byte[], VersionedValue> kv, long commitTimestamp) {
     ...
     */
    public CommitTimestamp locateCellCommitTimestamp(KetaTransaction transaction, long cellStartTimestamp,
    ...
    public Optional<Long> tryToLocateCellCommitTimestamp(KetaTransaction transaction,
                                                         KeyValue<byte[], VersionedValue> cell,
        ...
        CommitTimestamp tentativeCommitTimestamp =
            locateCellCommitTimestamp(transaction, cell.value.getVersion(),
                new CommitTimestampLocatorImpl(new KetaCellId(versionedCache, cell.key, cell.value.getVersion()),
    ...
    private Optional<Long> getCommitTimestamp(KetaTransaction transaction, KeyValue<byte[], VersionedValue> kv,
    ...
    public Optional<Long> getTSIfInTransaction(KetaTransaction transaction,
                                               KeyValue<byte[], VersionedValue> kv) {
        long startTimestamp = transaction.getStartTimestamp();
    ...
    public Optional<Long> getTSIfInSnapshot(KetaTransaction transaction,
                                            KeyValue<byte[], VersionedValue> kv,
                                            Map<Long, Long> commitCache)
        throws IOException {
    ...
    public List<VersionedValue> filterCellsForSnapshot(
        KetaTransaction transaction, byte[] key, List<VersionedValue> rawCells) throws IOException {
        ...
        for (VersionedValue value : rawCells) {
            KeyValue<byte[], VersionedValue> kv = new KeyValue<>(key, value);
    ...
    @Override
    public KeyValueIterator<byte[], List<VersionedValue>> all(KetaTransaction transaction) {
        return new FilteredKeyValueIterator(transaction, versionedCache.all(0, Long.MAX_VALUE));
    }

    @VisibleForTesting
    public boolean isCommitted(KetaTransaction transaction, KetaCellId cellId) throws TransactionException {
        try {
    ...
    private class FilteredKeyValueIterator implements KeyValueIterator<byte[], List<VersionedValue>> {
        private final KetaTransaction transaction;
        private final KeyValueIterator<byte[], List<VersionedValue>> iterator;

        FilteredKeyValueIterator(
            KetaTransaction transaction,
        ...
        public final KeyValue<byte[], List<VersionedValue>> next() {
            try {
                KeyValue<byte[], List<VersionedValue>> next = iterator.next();
